...
public class PrintRootToLeaf {
	...
	static void printTopToBottomPath(Node curr,
		...
		while (curr != null) {
			stk.push(curr);
			curr = parent.get(curr);//parent[curr];
		}
	...
	static void printRootToLeaf(Node root) {
		...
		/*
		 * Pop all items one by one. Do following for every popped item a) push
		 * its right child and set its parent pointer b) push its left child and
		 * set its parent pointer Note that right child is pushed first so that
